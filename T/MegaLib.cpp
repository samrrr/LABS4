...
BigPol operator/(const BigPol &_a, const BigPol &_b)
{
	...
	for (i = 0; i < _b.d.size(); i++)
	{
		b.d[i] = b.d[i] / b.d[_b.d.size()-1];
	}
	...
	while (ste >= 0)
	{
		r = _b.d.size();
		printf("%4i",ste);
		res.add_x(c.d[r - 1 + ste],  ste);
		for (i = 0; i < r; i++)
			c.d[i + ste] = c.d[i + ste] - b.d[i] * res.d[ste];
		//c[r-1+ste] = 0;
		ste--;
	}
...
BigPol POLY_NOD(BigPol _a, BigPol _b)
{
	...
	if (a.get_n() <= 1)
	{
		if (a == (BigPol)(string)("0"))
			return b;
		else
			return (BigPol)(string)("1");
	}

	if (b == (BigPol)(string)("0"))
		return a;
	else
		return (BigPol)(string)("1");
}
